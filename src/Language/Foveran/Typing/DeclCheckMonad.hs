...
import           Text.PrettyPrint as PP (render, (<+>), ($$), nest, (<>), vcat, text)
...
newtype DeclCheckM a = DM { runDM :: StateT Context (ExceptT (Span, Error) IO) a }
...
runDeclCheck e = do
  r <- runExceptT $ evalStateT (runDM e) emptyContext
  case r of
    Right () -> return ()
    ...
    Left (p, RepeatedIdent nm) ->
        putStrLn $ render $ "Repeated binding" <+> "“" PP.<> ppIdent nm PP.<> "”" <+> "at" <+> ppSpan p
    ...
    Left (p, DataDeclError e) ->
        putStrLn $ render $ "Data Declaration error at" <+> ppSpan p PP.<> ":" <+> ppDataDeclError e
